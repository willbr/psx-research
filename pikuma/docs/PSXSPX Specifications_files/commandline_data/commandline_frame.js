...
When adding updaters, don't forget to set("configversion", newversionnumber)!`, new FunctionType([], new TypeReferenceType("Promise", [new BooleanType(false, false)], false, false), false, false), true)],
  ...
  function focus() {
    ...
    logger8.debug("commandline_frame clInput focus(), activeElement is clInput: " + (window.document.activeElement === commandline_state.clInput));
  ...
  commandline_state.clInput.addEventListener(
    ...
    function(keyevent) {
      if (!keyevent.isTrusted)
        return;
      logger8.debug("commandline_frame clInput keydown event listener", keyevent);
  ...
  commandline_state.clInput.addEventListener("input", () => {
    logger8.debug("commandline_frame clInput input event listener");
    clInputValueChanged();
  });
  ...
  function fillcmdline(newcommand, trailspace = true, ffocus = true) {
    logger8.debug("commandline_frame fillcmdline(newcommand = " + newcommand + " trailspace = " + trailspace + " ffocus = " + ffocus + ")");
    if (trailspace)
  ...
  logger8.debug("Added commandline_frame message listener");
